...
class AppManager(private var mApplication: Application?) {
    protected val TAG = AppManager::class.java.name
...
//                showSnackbar(message.obj as String, if (message.arg1 === 0) false else true)
            }
            KILL_ALL -> killAll()
            APP_EXIT -> appExit()
            else -> {
    ...
    fun killAll(vararg excludeActivityClasses: Class<*>) {
        ...
        synchronized(AppManager::class.java) {
            ...
            while (iterator.hasNext()) {
                ...
                if (excludeList.contains(arrayOf(next::class.java)))
                    continue
    ...
    fun killAll(vararg excludeActivityName: String) {
        ...
        synchronized(AppManager::class.java) {
            ...
            while (iterator.hasNext()) {
                ...
                if (excludeList.contains(arrayOf(next.javaClass.name)))
                    continue
